...
impl Scopes {
    ...
    ///
    /// ```
    /// use mastodon_async::scopes::{AdminRead, Scopes};
    ///
    ...
    ///
    /// ```
    /// use mastodon_async::scopes::{Scopes, AdminWrite};
    ///
...
/// [1]: https://docs.joinmastodon.org/api/oauth-scopes/#list-of-scopes
...
impl FromStr for AdminRead {
    ...
    fn from_str(s: &str) -> Result<AdminRead, Self::Err> {
        Ok(match s {
            ...
            _ => return Err(Error::Other("Unknown 'admin:read' subcategory".to_string())),
        })
    }
}
